...
当前配置表清单和用途概览见 `CONFIG_TABLES.md`。
...
- 新增配置表、删除配置表、修改配置表结构（字段增删、字段类型变化、字段语义变化）或显著改变配置表用途时，必须检查并按需更新 `CONFIG_TABLES.md`。
- 只修改已有表的数据内容且不改变用途或结构时，可不更新 `CONFIG_TABLES.md`。
