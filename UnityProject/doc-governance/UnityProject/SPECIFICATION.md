...
- 2026-05-25：`UserInfoPopup` 新增最近对局摘要区，从 `save/replay/ReplayIndex.json` 读取可见复盘归档并按索引顺序展示。列表是固定元素分页，不使用滚动列表；条目由 `ReplayArchiveItemWidget.prefab` 实例化，按当前列表区域高度自适应计算每页条数且最多 7 条，显示棋盘大小、对局来源、胜负方、最近更新时间、手数、终局方式和结果文本，不绘制终局盘面。UI 只展示索引中标记为 `isArchived` 的有效记录，短局阈值由复盘归档/索引写入逻辑负责。上一页/下一页按钮只切换当前页；当前点击条目仅提示复盘浏览稍后开放，不加载复盘场景。
