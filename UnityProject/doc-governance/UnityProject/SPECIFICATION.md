...
- 2026-05-19：`DuelPage.prefab` 通过 Binder 显式维护“虚手”“请求数子”和“认输”按钮；业务代码只绑定点击监听，不在运行时创建这些固定 UI 控件。请求数子会由 `DuelSystem` 按本地棋盘状态执行原型中国数子：黑白双方分数分别由己方棋子数、完全被单方包围的空点数构成，白方额外加 `KataGoDuelRecordFile.Komi` 当前固定 7.5 贴目；系统据此判断胜者和目差，并通过通用 `ConfirmPopup` 要求玩家确认，确认后写入 `SceneComponentDuel.finalBlackScore`、`finalWhiteScore`、`finalScoreMargin`、`winnerGuid` 和 `gameEndReason`，再进入 `GameEnd`。虚手会写入 KataGo 标准 `moves` 项 `["B","pass"]` 或 `["W","pass"]`，第一手虚手只推进回合并清除旧形势图层，双方连续虚手会立即按同一数子算法结束对局，不弹出确认。认输会先通过通用二次确认，确认后写入 `resignLoserGuid`、`winnerGuid` 和 `gameEndReason` 并进入 `GameEnd`。该原型数子当前不包含死子标记、死子确认或让子计分。
...
- AI difficulty options are table-driven by `Assets/Config/DataJson/duel_ai_difficulty/duel_ai_difficulty.json`. `DuelSetupPopup` displays the config `name` values in a fixed rank order and passes the selected config id through `DuelSceneCreateParamas`. The difficulty table includes base selection parameters and board-size-specific realtime overrides for 9x9, 13x13, and 19x19 boards.
...
- `DuelAiSystem` is installed in `DuelScene`. During an AI turn it asks KataGo analysis for move candidates using the current KataGo-standard `moves`, filters candidates through the same local move legality rule path, then emits the normal `OnAddChessToBoard` event. Human board click, pass, and resign inputs are ignored while the current turn belongs to the AI player. Realtime AI move requests resolve `maxVisits`, candidate count, and maximum score-loss threshold from the selected board size; request visits use `min(maxVisits, realtimeMaxVisitsN)` so high-rank configs keep their selection behavior without monopolizing the single local analysis process.
...
- `OnRequestDuelPass` 在回合输入状态下记录当前玩家虚手并推动回合结束；双方连续虚手会直接数子并进入 `GameEnd`。
- `OnRequestDuelScore` 会根据当前棋盘自动数子并弹出二次确认；玩家确认后进入 `GameEnd`，取消则继续当前对局。
...
- 死子确认、复盘、匹配、房间、重连和网络同步当前未实现；数子、虚手、认输和基础终局结果 UI 已有本地原型实现，但尚未覆盖死子确认或完整线上裁定模型。
