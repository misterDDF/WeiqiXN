...
- 2026-05-19：合法落子成功后会向 `SceneComponentDuel.kataGoMoves` 追加 KataGo 标准手顺数组项，例如 `["B","Q16"]`、`["W","D4"]`。本地 `RectCoordinates` 直接采用 KataGo 棋盘布局作为逻辑坐标契约：`x` 从左到右递增，`z` 从棋盘上边向下递增；9 路左上角为本地 `(0,0)` 并写为 `A9`，左下角为本地 `(0,8)` 并写为 `A1`。KataGo `ownership` 数组按同一行序直接回绘，不再在 KataGo 边界做额外坐标兼容转换。`DuelSaveSystem` 保存对局检查点时先写入 `GameSaveConfig.GetDuelRecordSavePath(0)` 指向的 KataGo analysis JSON 记录文件，再保存场景状态；记录文件包含 `boardXSize`、`boardYSize`、`rules`、`komi`、`initialStones`、`moves`、`includeOwnership: true` 和 `includePolicy: false`，可直接作为第一版 ownership 分析请求骨架；分先对局的 `initialStones` 为空，让子棋会写入预置黑子。当前正式流程暂不包含读档/继续对局入口。`KataGoPositionJsonBuilder.BuildOwnershipAnalysisJson` 是第一版形势按钮默认入口，优先使用当前对局的 `moves` 生成 ownership 请求；`BuildAnalysisJsonWithCurrentBoard` 只保留为调试或无手顺场景的快照入口。
- 2026-05-25：复盘归档写入时机已从手动保存按钮中分离。`DuelReplayArchiveSystem` 监听成功落子、成功虚手、成功悔棋、数子失败回滚和进入终局事件；当前对局第一次出现有效手顺时生成 `SceneComponentDuel.replayGameId`、`replayCreatedAtUtc` 和 `replayLastUpdatedAtUtc`，后续同一局只覆盖更新同一 `save/replay/{gameId}/` 目录，不因悔棋、虚手或继续落子拆成多条。复盘目录保存 `DuelScene.json`、`DuelRecord.json` 和 `SaveInfo.json`；`SaveInfo.json` 额外记录 `gameId`、创建/更新时间、归档时间、手数、是否完成、是否归档、来源类型、黑白双方显示名、胜者座位、终局结果、棋盘、贴目、用时和让子摘要。总手数小于 16 手的对局只允许存在草稿目录，不写入 `save/replay/ReplayIndex.json`；短局终局会移除索引项并删除草稿目录。`ReplayIndex.json` 只收录 16 手及以上的可见复盘归档，并按 `lastUpdatedAtUtc` 倒序维护条目，索引摘要冗余黑白双方显示名以供最近对局列表直接展示。`save/0/` 仍作为手动保存按钮写入的运行中检查点路径，不作为最近复盘列表的数据来源。
...
- 2026-05-25：`UserInfoPopup` 新增最近对局摘要区，从 `save/replay/ReplayIndex.json` 读取可见复盘归档并按索引顺序展示。列表是固定元素分页，不使用滚动列表；条目由 `ReplayArchiveItemWidget.prefab` 实例化，按当前列表区域高度自适应计算每页条数且最多 7 条，显示黑方/白方姓名、棋盘大小、对局来源、胜负方、最近更新时间、手数、终局方式和结果文本，不绘制终局盘面。UI 只展示索引中标记为 `isArchived` 的有效记录，短局阈值由复盘归档/索引写入逻辑负责。上一页/下一页按钮只切换当前页；当前点击条目仅提示复盘浏览稍后开放，不加载复盘场景。
...
- `MainMenuScene` 加载后打开 `MainMenuPage`；主菜单的个人信息入口打开 `UserInfoPopup`，可查看本地姓名、ID、胜场、负场和最近对局摘要列表，并通过确认弹窗输入模式修改姓名。
...
- `DuelScene` 创建 `SceneComponentChessBoard` 和 `SceneComponentDuel`，从 `DuelSceneFixedRef` 绑定固定场景引用，安装 `DuelSaveSystem`、`ChessBoardSystem`、`DuelOwnershipSystem`、`DuelAuthoritySystem`、`DuelInputAuthoritySystem`、`DuelAiSystem`、`DuelSystem`、`LanDuelSystem`、`DuelReplayArchiveSystem`，然后打开 `DuelPage`。
...
- `DuelSaveSystem` 响应 `OnSaveDuelScene`，先通过 `KataGoDuelRecordFile.Save` 保存棋盘记录文件，再通过 `DuelSaveInfoFile.Save` 保存槽位摘要信息，成功后再通过 `GameSaveManager.SaveDataAsync` 把当前场景状态保存到 `GameSaveConfig.GetDuelSceneSavePath(0)`；任一步失败或全部成功后都会发出 `OnDuelSaveResult`。该手动保存路径是运行中检查点，不进入复盘索引。
- 保存使用 `SavableObj`、`SavableField`、可保存集合和 JSON 文件；Unity Editor 下存档根目录是仓库根目录的 `save/`，PC Standalone 下存档根目录是游戏包体根目录的 `save/`，其他非 Editor 平台默认使用 `Application.persistentDataPath`；对局槽位目录为 `save/0/`、`save/1/` 等，复盘归档目录为 `save/replay/{gameId}/`。`SaveInfo.json` 记录存档时间、槽位、棋盘配置、时间配置、让子配置、当前手数和复盘列表摘要字段。读档/继续对局暂不作为当前正式功能。
