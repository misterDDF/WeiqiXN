...
- 涉及 Unity C# 文件改动的任务，只读搜索、读取代码、查看 diff 或分析调用链不需要 Play 模式状态检查；准备执行 `apply_patch`、脚本生成、格式化或其他会写入 C# 文件的操作前才必须完成状态检查。优先通过 MCP 执行 `自定义功能/Editor/Write Editor State`，再读取 `UnityProject/Temp/WeiqiXN/editor_state_probe.json`，以文件中的最新 `timeUtc` / `sequence`、`isPlaying`、`isPlayingOrWillChangePlaymode` 和 `isCompiling` 为准，不依赖 Console 日志。发现 Editor 正在 Play 时，不执行任何会改写 C# 文件的操作，先告知用户需要退出 Play。C# 写入完成后应立即执行必要 Unity 导入/编译验证，再进入较长文档同步；若写入后未立即验证而发生文档维护、等待、用户交互或其他明显间隔，则在导入、重编译或任何可能触发 Domain Reload 的 Editor 操作前必须重新探针确认非 Play。
