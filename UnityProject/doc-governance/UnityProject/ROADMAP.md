...
- Added the first in-game OGS login entry on `UserInfoPopup`: the OGS login button uses the same browser callback login service, saves the authenticated OGS username as the local display name, refreshes the popup, and resyncs the LAN player profile. The current runtime callback is still desktop/Editor localhost based; mobile deep-link login remains a follow-up.
