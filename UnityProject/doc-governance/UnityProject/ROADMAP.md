...
- 做出联机架构决策：传输或框架选择、权威模型、房间或会话模型、落子协议、重连策略、持久化预期。当前已收敛为 host 权威、单一 server core、客户端命令驱动的方案，下一步只需要把最小协议和快照结构细化到可实现粒度。
...
- 下一步如果继续沿计划推进，应进入 P3 联机架构决策收口：在 host 权威、单一 server core 的前提下，先细化最小命令协议、快照结构和 Unity/网络边界，再决定是否进入最小联机原型。
...
- 阶段 2 目标：联机基础。第一交付物是基于 host 权威、单一 server core 的架构决策和最小落子命令协议，不是完整匹配产品。
