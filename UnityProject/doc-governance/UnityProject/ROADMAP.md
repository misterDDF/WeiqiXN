...
- 做出联机架构决策：传输或框架选择、权威模型、房间或会话模型、落子协议、重连策略、持久化预期。当前已收敛为 host 权威、单一 server core、客户端命令驱动的方案；最小房间、正常落子、虚手、认输、确认式数子、确认式悔棋、主动离开、棋盘版本、落子后快照纠偏、host 下发输入权、host 开局座位配置和 host 权威计时已进入原型，下一步需要补齐断线恢复和完整终局恢复边界。
...
- 当前主线已进入局域网联机原型：已有 UDP 房间发现、TCP 加入握手、准备/开局、正常落子命令、统一对局命令提交入口、棋盘版本、落子后快照纠偏、host 下发输入权、host 权威计时、虚手、认输、确认式数子、确认式悔棋和 `LeaveRoom` 主动离开；仍没有新增第三方网络 SDK、匹配、重连或完整线上裁定实现。
- P2.5 复盘主线已从 UI-only 文本棋盘切到独立 `ReplayScene`：最近对局条目进入复盘专用场景，复用对局棋盘资源、棋盘系统和落子规则回放，`ReplayPage` 只保留控制层。后续仍需补齐折线图跳转、试下模式和 AI 推荐展示。
- 下一步如果继续沿计划推进，应继续收口 P3 联机原型：在 host 权威、单一 server core 的前提下，补齐断线恢复、完整终局恢复和手动双端回归验证。
...
- 当前已新增 KataGo 标准棋谱链路和第一版形势按钮链路：合法落子直接维护 KataGo `moves`，让子棋的预置黑子输出到 KataGo `initialStones`，保存对局时生成可直接作为 ownership analysis 请求骨架的记录 JSON；`DuelPage` 右下角“形式”按钮会请求当前对局 `ownership`，绘制棋盘 overlay，并在按钮上方显示黑方目数和白方贴目后目数；当前盘面快照式 `initialStones` 入口仅保留为调试或无手顺场景。读档/继续对局暂不作为当前正式功能。
