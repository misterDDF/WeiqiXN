...
- 当前已接入 Play 模式和 Windows PC 包启动 smoke test：启动 Loading 阶段优先使用 `opencl` 引擎，后台加载模型并依次验证 9 路、13 路、19 路 `ownershipLength` 日志；OpenCL 缺失、启动失败或任一 smoke test 失败时会自动 fallback 到 `eigenavx2` 引擎。启动时会检查游戏根目录写权限；不可写时会弹窗提示、跳过 OpenCL，并使用 CPU 引擎的 `analysis_nowrite.cfg` 关闭 KataGo 文件写入。
