...
- 做出联机架构决策：传输或框架选择、权威模型、房间或会话模型、落子协议、重连策略、持久化预期。当前已收敛为 host 权威、单一 server core、客户端命令驱动的方案；最小房间、正常落子、虚手、认输、确认式数子、确认式悔棋、棋盘版本、落子后快照纠偏、host 下发输入权、host 开局座位配置和 host 权威计时已进入原型，下一步需要补齐断线恢复和完整终局恢复边界。
...
- 当前主线已进入局域网联机原型：已有 UDP 房间发现、TCP 加入握手、准备/开局、正常落子命令、统一对局命令提交入口、棋盘版本、落子后快照纠偏、host 下发输入权、host 权威计时、虚手、认输、确认式数子和确认式悔棋；仍没有新增第三方网络 SDK、匹配、重连或完整线上裁定实现。
