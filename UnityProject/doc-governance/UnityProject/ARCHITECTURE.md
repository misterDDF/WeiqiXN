...
- `ReplayPage` 是与 `ReplayScene` 生命周期绑定的 Header 主页面，只负责类悬浮 HUD 控制层、按钮命令转发和状态展示；棋盘渲染、提子、最新手标记和局面重建都归属 `ReplayScene` 与棋盘表现链路，不允许回退成页面内文本棋盘或页面内棋规实现。
...
- 不要在复盘 UI 页面内实现棋规、提子或棋盘渲染；复盘必须走独立场景和对局棋盘表现链路，UI 只驱动浏览游标和展示状态，并保持不遮挡棋盘主体区域的 HUD 形态。
