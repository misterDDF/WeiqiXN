...
- 2026-05-29: Native KataGo request concurrency is a shared Unity-side analysis boundary for Windows and Android native backends. `KataGoBootstrap` owns the configured native request gate, and backend wrappers only report whether their loaded bridge binary supports concurrent analyze. The bridge concurrency contract is single analysis engine, multiple in-flight JSON requests, and request-id based output dispatch; callers must not create multiple native bridge engines in one Unity process because the bridge still redirects KataGo analysis through process-wide standard streams.
