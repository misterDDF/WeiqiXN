...
- 2026-06-02: `OgsConnectionService` is the first OGS integration boundary. It is a global service for OAuth2 PKCE login, token/session persistence, token refresh, logout, and minimum authenticated REST probes against OGS `/api/v1/me/` and UI config endpoints. It does not participate in `DuelScene`, `DuelAuthoritySystem`, `LanRoomService`, or local/LAN host adjudication yet. OGS game integration must continue to treat OGS server state as the remote authority and adapt accepted server state into the existing submit/accepted/snapshot/input-authority presentation path instead of routing OGS moves through the local/LAN host core.
