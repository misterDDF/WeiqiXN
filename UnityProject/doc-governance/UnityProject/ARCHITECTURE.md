...
- 2026-06-02: `UserInfoPopup` has the first in-game OGS login entry. It calls `OgsConnectionService.LoginWithBrowserCallbackAsync`, then updates the local display name from the OGS session username and resyncs the LAN player profile. This runtime entry currently uses the desktop localhost callback path; Android/mobile login still requires a later deep-link or custom-scheme callback design.
