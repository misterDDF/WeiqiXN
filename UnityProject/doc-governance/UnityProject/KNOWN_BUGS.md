...
### LAN 创建房间后仍可搜索到本机房间且关闭弹窗释放不稳定

- 状态：修复实现已落地，待联机复测后移除
- 记录日期：2026-05-26
- 涉及范围：LAN 房间弹窗、局域网房间发现、房主广播、弹窗关闭释放
- 已观察行为：创建 LAN 房间后仍可点击搜索房间，并在搜索列表中看到自己创建的房间；创建房间后通过非关闭按钮路径关闭联机窗口时，房间广播、监听或连接资源可能没有随弹窗关闭释放。
- 可见表现：同一客户端同时处于房主广播和搜索状态，用户可能误点自己的房间；窗口关闭后局域网内仍可能发现该房间或端口仍被占用。
- 期望行为：创建、加入和搜索状态互斥；已有 LAN 会话时不应再启动搜索；搜索发现列表不显示本机正在广播的房间；弹窗在未进入对局前关闭时必须释放搜索、房间监听、UDP 广播和 TCP 连接。
- 当前处理：已让 `LanRoomService.StartSearchRooms()` 在已有 LAN 会话时拒绝启动搜索；发现房间写入列表前按本机 `roomId` 过滤；`LanRoomPopup.OnClose()` 在未进入 LAN 对局时兜底调用 `LeaveCurrentSession()`，不再只依赖关闭按钮回调。
- 移除条件：完成 Unity 脚本编译，并通过本机或双端 LAN 复测确认创建房间后无法搜索到本机房间、关闭弹窗后广播和监听释放，再移除此条记录。
