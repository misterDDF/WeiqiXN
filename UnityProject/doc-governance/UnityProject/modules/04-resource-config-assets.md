...
- `ui_page.json` 已配置 LoadingPage、MainMenuPage、UserInfoPopup、ReplayPage、DuelSetupPopup、LanRoomPopup、DuelPage、DuelMoveConfirmPopup、ConfirmPopup、OgsFriendListPopup、RecentReplayListPopup 和 OgsFriendProfilePopup。
...
- The runtime asset table is consumed by `AssetBundleGenerator.PackRuntimeAssetTable`, which validates each listed asset and sets the configured AssetBundle label before building bundles. Runtime asset rows may use an explicit file extension in `resPath`, such as `.mp3` or `.wav`, when the runtime load path also includes the extension.
- The current runtime asset entries include the shared black and white chess-board materials used by ownership overlay/latest-move marker rendering, plus code-loaded audio clips used by `GameAudio`, including the duel voice preview clips under `Assets/Audio/Voice/OgsPreview/*.wav`.
- LAN room runtime parameter configs are exported from `ConfigExporter/xlsx/lan_room_config.xlsx`. The exported JSON lives under `Assets/Config/DataJson/lan_room_config/`, and the generated data type lives under `Assets/Config/DataType/lan_room_config/`. The table contains ports, timeout, max player count, broadcast interval, buffer sizes, heartbeat interval, heartbeat timeout, and reconnect probe interval; LAN protocol strings are code-owned by `LanRoomProtocol`.
...
- Chess stone visuals are maintained by `Assets/Models/Chess/ChessBlack.prefab`, `Assets/Models/Chess/ChessWhite.prefab`, their preview variants `Assets/Models/Chess/ChessBlackPreview.prefab` and `Assets/Models/Chess/ChessWhitePreview.prefab`, their materials under `Assets/Models/Chess/Materials/`, and the URP shader `Assets/Models/Chess/Shaders/StoneGloss.shader`. The editor menu `Tools/WeiqiXN/Polish Chess Stone Assets` reapplies the current stone material parameters and flattened stone prefab transform.
- Runtime chess stone prefabs keep the root transform aligned to board intersections; visual-only randomness is applied under the `VisualOffset` child by `ChessStoneVisualRandomizer`, while the drop animation targets `VisualOffset/Model`.
