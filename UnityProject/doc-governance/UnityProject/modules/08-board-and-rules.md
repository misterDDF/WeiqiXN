...
- 当前“请求数子”和双方连续虚手只依赖 KataGo `ownership` 统计结果；本地棋盘状态数子算法不再作为回退路径。该口径仍未覆盖死子确认或完整线上裁定模型。
...
- 为 KataGo 分析提供只读 JSON 构造：正常路径直接使用运行时 `moves`；让子棋预置黑子通过配置生成 `initialStones`，不写入 `moves`；当前棋盘快照转换为 `initialStones` 只用于调试或无手顺场景，不在转换过程中修改棋盘状态。
