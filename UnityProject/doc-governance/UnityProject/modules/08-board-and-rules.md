...
- `SceneComponentChessBoard` 负责棋盘配置 id、当前棋子信息、上一局面棋子信息、棋盘引用和虚拟相机引用。
- `ChessBoardSystem.Init()` 根据棋盘配置初始化网格，设置相机俯视棋盘，并在读档时恢复棋子实体。
...
- KataGo 编辑器验证尚未进入当前规则实现；它应读取棋盘快照做 AI 形势分析，而不是参与落子合法性或正式计分。
- 合法落子成功后会记录一条 KataGo 可读手顺，当前格式为 `颜色:坐标`，例如 `B:Q16`；该记录随 `SceneComponentDuel` 存档。
...
- 在 UI 层订阅非法原因并显示提示。
- 为 KataGo 分析提供只读转换：完整手顺转换为 analysis engine 的 `moves`，当前棋盘快照转换为 `initialStones`。形势按钮优先使用完整手顺；快照入口只用于兜底和调试，不在转换过程中修改棋盘状态。
