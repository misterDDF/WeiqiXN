...
- `OgsConnectionService` now exists as a global service for OGS OAuth2 PKCE login preparation, minimum authenticated REST connectivity, realtime authentication, default bot-game smoke creation, and realtime automatch game start.
- The service can create an authorization URL, exchange an authorization code with a PKCE verifier, refresh a token, clear a local session, query the current OGS user through `/api/v1/me/`, request OGS UI config for the realtime `user_jwt`, connect to a game for `gamedata`, create a default OGS bot challenge for smoke/testing, start OGS realtime automatch, and create a persistent realtime game session for project gameplay. Tokens are persisted in the save root as an OGS session file and are not written to diagnostic logs. The default OAuth scope is now `read write`; existing read-only sessions must re-login before creating or matching a game.
...
- `UserInfoPopup` now exposes the first in-game OGS login entry. A successful OGS login updates the local user display name from the OGS session username and syncs the LAN local player profile. This is still a desktop/Editor localhost callback path; Android/mobile login needs a later deep-link or custom-scheme callback.
