...
- `MainMenuPage` now exposes the first OGS game entry: `btn_ogs_game` is hidden until an OGS session exists, then creates a default 9x9 OGS bot challenge, sends realtime `game/connect`, waits for server `gamedata`, and enters `OgsDuelScene`.
...
- `ogs_config` carries OGS OAuth, REST, realtime, timeout, and default 9x9 bot-game values in the same table-driven style as `lan_room_config`. The source table lives in `ConfigExporter/xlsx/`; generated JSON and C# config types are consumed by Unity.
