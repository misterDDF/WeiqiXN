...
联机功能已开始进入最小房间和最小落子命令阶段。当前已有 `LanRoomService` 作为局域网房间服务入口，支持 host 侧 TCP 监听、UDP 房间广播、client 侧 UDP 搜索、TCP 加入握手、最小准备状态交换、host 开局配置、进入带 LAN 标记的 `DuelScene`，以及 `SubmitMove` / `MoveAccepted` / `MoveRejected` / `BoardSnapshot` 的最小命令搬运；正常落子已通过 `OnSubmitDuelMove` 和 `DuelAuthoritySystem` 收敛为统一提交入口，页面预览、点击提交和 LAN 提交前置权限通过 `DuelInputAuthority` 读取，host 侧 `LanDuelSystem` 通过现有规则入口进行权威落子校验，并在合法落子后广播权威棋盘快照。认输已接入 `SubmitResign` / `ResignAccepted`，host 只接受当前行棋方认输并广播同一终局结果。host 也负责广播当前行棋方 `TimeState` 并在超时后广播 `PlayerTimeout`，client 只使用 host 计时状态刷新显示。尚未实现正式传输抽象、座位选择、匹配系统、完整同步系统或断线恢复系统。架构方向已收敛为 host 权威、单一 server core、客户端只发命令；第一版 server core 允许嵌在 host 进程中运行，后续再决定是否拆成独立进程。
...
- 本端人类落子权限已收敛到 `DuelInputAuthority`：当前 LAN 原型按 host=Player1、client=Player2 派生输入方，页面预览、点击提交和 LAN 提交前置检查使用同一份权限状态；后续可替换为 host 下发的控制状态。
...
- 还没有完整服务端或可独立运行的房主权威会话核心；当前只完成房间 host 监听、加入握手、准备状态、开局命令、正常落子、计时、超时和认输的内嵌 host 权威原型。
- 移动命令协议已有最小正常落子命令、版本确认和认输接受，尚未覆盖虚手、数子或悔棋。
- 棋盘状态快照只覆盖落子后的棋盘纠偏，计时只覆盖在线显示校准和超时终局通知，尚未覆盖断线重连、房间完整状态或终局恢复。
- 没有玩家身份、座位选择和完整房间生命周期。
- 没有断线、重连、虚手、数子、悔棋和完整终局恢复在网络环境下的定义；超时和认输已有最小 host 权威接受路径。
- 没有防作弊边界。
...
4. 定义最小协议：`JoinRoom`、`Ready`、`StartConfig`、`SubmitMove`、`MoveAccepted`、`MoveRejected`、`BoardSnapshot`、`TimeState`、`PlayerTimeout`、`SubmitResign`、`ResignAccepted`、`LeaveRoom`、`Heartbeat`。
