...
- `OgsConnectionService` now exists as a global service for OGS OAuth2 PKCE login preparation, minimum authenticated REST connectivity, realtime authentication, and default 9x9 bot-game creation.
- The service can create an authorization URL, exchange an authorization code with a PKCE verifier, refresh a token, clear a local session, query the current OGS user through `/api/v1/me/`, request OGS UI config for the realtime `user_jwt`, connect to a game for `gamedata`, create a default 9x9 OGS bot challenge, and create a persistent realtime game session for project gameplay. Tokens are persisted in the save root as an OGS session file and are not written to diagnostic logs. The default OAuth scope is now `read write`; existing read-only sessions must re-login before creating a game.
...
- `MainMenuPage` now exposes the first OGS game entry: `btn_ogs_game` is hidden until an OGS write-capable session exists. Clicking it first queries the current user's active OGS games and enters `OgsDuelScene` directly if one can be loaded. If no active game exists, it opens `DuelSetupPopup` in OGS mode; the popup hides AI difficulty, keeps OGS side/board/time/byoyomi/handicap setup available, filters those table options by `ogsEnabled`, and then creates the current bot-test challenge from the selected parameters before sending realtime `game/connect`, waiting for server `gamedata`, and entering `OgsDuelScene`.
