...
- 数子和悔棋已接入确认式 host 权威路径：发起方提交到 host，host 转发确认请求给对端，对端确认后 host 执行并广播 `ScoreResult` / `ScoreFailed` 或 `TakeBackAccepted` / `TakeBackRejected`。
