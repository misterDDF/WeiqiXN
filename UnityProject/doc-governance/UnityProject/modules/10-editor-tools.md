...
- `ClientMain` 按普通流程调用 `KataGoBootstrap.Start()` / `Stop()`；`KataGoBootstrap` 内部按平台解析引擎路径。Windows Unity Editor 和 Windows PC 包会通过 `Application.streamingAssetsPath/KataGo` 后台启动 Eigen AVX2 版 KataGo，加载本地模型，发送固定 smoke query，并在日志中输出第一版所需的 `ownershipLength`。
