...
- `ClientMain` 按普通流程调用 `KataGoBootstrap.Start()` / `Stop()`；`KataGoBootstrap` 内部按平台解析引擎路径。Windows Unity Editor 和 Windows PC 包会通过 `Application.streamingAssetsPath/KataGo` 定位资源，并通过同一套 Win32 pipe 子进程适配器优先后台启动 OpenCL 版 KataGo，加载本地模型，发送固定 smoke query，并在日志中输出第一版所需的 `ownershipLength`；OpenCL 缺失、启动失败或 smoke test 失败时会自动 fallback 到 Eigen AVX2 CPU 版 KataGo。
