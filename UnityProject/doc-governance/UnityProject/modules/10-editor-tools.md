...
- Windows `native-opencl/analysis_example.cfg` 当前按本机 smoke 结果优先降低单次分析延迟：`numAnalysisThreads=1`、`numSearchThreadsPerAnalysisThread=8`、`nnMaxBatchSize=16`。该配置只调整 KataGo 内部搜索线程和 NN batch，不改变 `KataGoBootstrap` 现有串行请求门控；Android native 配置不跟随该 PC 参数。
