...
- `ClientMain` 按普通流程调用 `KataGoBootstrap.Start()` / `Stop()`；`KataGoBootstrap` 内部按平台解析引擎路径。Windows Unity Editor 和 Windows PC 包会通过 `Application.streamingAssetsPath/KataGo` 定位资源，并通过同一套 Win32 pipe 子进程适配器后台启动 Eigen AVX2 版 KataGo，加载本地模型，发送固定 smoke query，并在日志中输出第一版所需的 `ownershipLength`。
...
- KataGo 二进制、模型和配置路径位于 `Assets/StreamingAssets/KataGo/`；Windows PC 构建会把该目录复制到包体 `StreamingAssets`。缺失、启动失败、超时和协议解析失败都需要有明确日志；Windows 打包入口会在构建前校验当前 `eigenavx2` 运行所需文件是否齐全。
