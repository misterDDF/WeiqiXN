...
- `DuelSetupPopup` 默认选择无限持有时间；本地对局隐藏开局座位选择但保留让子下拉框，电脑对局和 LAN 创建房间显示 `猜先` / `执黑` / `执白`。`猜先` 强制分先并禁用让子选择，执黑或执白时可按棋盘尺寸选择让先或让 2 子到最大让子数。OGS 创建对局也复用该弹窗，使用独立的 OGS mode state 隐藏 AI 难度、显示座位选择，并按 `chess_board`、`duel_hold_time`、`duel_byoyomi_count`、`duel_byoyomi_time` 和 `duel_handicap` 表中的 `ogsEnabled` 字段筛选可用选项；当前 OGS 默认配置为 9 路、10 分钟、5 次 30 秒读秒，`infinite` 持有时间不在 OGS 模式启用。弹窗关闭时会按本地对局、电脑对局、LAN 创建房间和 OGS 创建对局四种模式分别保存当前选择到用户存档，下一次打开同模式弹窗时恢复最近一次配置，并对当前配置表中已不存在或当前模式不可用的 id 回退默认值。
...
- The OGS main-menu entry checks for an existing active OGS game before opening setup. If an active game is found, Unity loads it directly; otherwise `DuelSetupPopup.OpenForOgs` collects board, side, handicap, main time, byoyomi count, and byoyomi period before the current bot-test creation path submits an OGS challenge. OGS challenge creation maps byoyomi-enabled settings to OGS `byoyomi`, maps `off` byoyomi to OGS `absolute`, and keeps OGS as the authority after the created game returns `gamedata`.
