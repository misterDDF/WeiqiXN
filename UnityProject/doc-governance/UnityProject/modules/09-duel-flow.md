...
- `DuelPage.prefab` 会在形势按钮旁提供“虚手”入口；`DuelSystem` 在回合输入状态收到虚手后记录 KataGo `pass`，第一手虚手推进到下一回合，双方连续虚手会立即按 KataGo `ownership` 优先、本地原型数子兜底的结算结果进入 `GameEnd`，不弹二次确认。
- `DuelPage.prefab` 设置面板会提供“请求数子”和“认输”入口；`DuelSystem` 优先请求 KataGo `ownership`，复用形势按钮的阈值和贴目口径自动计算黑白分数、胜者和目差；KataGo 不可用或无结果时回退到本地原型数子。结果通过 `OnDuelScoreResult` 交给页面弹出通用二次确认，确认后进入 `GameEnd`，取消则保持当前对局。认输按钮只在回合输入且当前行棋玩家有效时显示，点击后先弹出通用二次确认，确认后当前行棋方判负并进入 `GameEnd`。
...
- `WaitAction` 未接入主路径。
- `GameEnd` 已可由超时、确认数子、双方连续虚手或认输进入，并保存 `winnerGuid`、终局原因和终局分数；当前数子优先复用 KataGo `ownership` 口径，仍缺少死子确认流程和线上裁定模型。
- 玩家显示仍偏调试形态。
...
- `GameEnd` is now reachable through timeout loss, scoring, consecutive pass, and resign; scoring currently prefers KataGo `ownership` with local prototype scoring as fallback, while dead-stone confirmation, review flows, and online adjudication remain out of scope.
