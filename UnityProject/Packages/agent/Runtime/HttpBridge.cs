                ...
                if (pending.streamHandler != null)
                {
                    ...
                    if (chunk != null)
                    {
                        ...
                        if (!pending.headerSent)
                        {
                            ...
                            try
                            {
                                ...
                                string statusText = request.responseCode >= 400
                                    ? (request.error ?? $"HTTP {(int)request.responseCode}")
                                    : (request.error ?? "OK");
                    ...
                    try
                    {
                        ...
                        if (!pending.headerSent)
                        {
                            ...
                            string statusText = request2.responseCode >= 400
                                ? (request2.error ?? $"HTTP {(int)request2.responseCode}")
                                : (request2.error ?? "OK");
                            sb.Append($"\"statusText\":\"{EscapeJsonString(statusText)}\",");
                            AppendHeaders(sb, responseHeaders);
                        ...
                        else if (request2.responseCode >= 400)
                        {
                            string responseBody = finalChunk ?? "";
                            Debug.LogError($"[HttpBridge] Stream request returned HTTP {(int)request2.responseCode} for {pending.url}: {responseBody}");
                            pending.callback?.Invoke($"{{\"error\":\"HTTP {(int)request2.responseCode}: {EscapeJsonString(responseBody)}\"}}");
                        }
                        else
                        {
