...
namespace LLMAgent.Editor
{
    ...
    public class AgentChatWindow : EditorWindow
    {
        ...
        private string textVerbosity = "high";
        ...
        private const string PrefKeyWireApi = "LLMAgent_WireApi";
        private const string PrefKeyReasoningEffort = "LLMAgent_ReasoningEffort";
        private const string PrefKeyTextVerbosity = "LLMAgent_TextVerbosity";

        private const string InputControlName = "AgentChatInput";
        private const float SettingsFieldHeight = 24f;
        ...
        private void LoadSettings()
        {
            ...
            wireApi = NormalizeWireApi(EditorPrefs.GetString(PrefKeyWireApi, "responses"));
            reasoningEffort = EditorPrefs.GetString(PrefKeyReasoningEffort, "high");
            textVerbosity = EditorPrefs.GetString(PrefKeyTextVerbosity, "high");
        }
        ...
        private void SaveSettings()
        {
            ...
            EditorPrefs.SetString(PrefKeyWireApi, wireApi);
            EditorPrefs.SetString(PrefKeyReasoningEffort, reasoningEffort);
            EditorPrefs.SetString(PrefKeyTextVerbosity, textVerbosity);
        }
        ...
        private string NormalizeBaseURL(string value)
        {
            ...
            if (normalized.EndsWith(ResponsesSuffix, StringComparison.OrdinalIgnoreCase))
                normalized = normalized.Substring(0, normalized.Length - ResponsesSuffix.Length);

            if (string.Equals(normalized, "https://api.openai.com", StringComparison.OrdinalIgnoreCase))
        ...
        {
            return string.Equals(value, "chat_completions", StringComparison.OrdinalIgnoreCase)
                ? "chat_completions"
                : "responses";
        }
        ...
        {
            string endpointBase = string.IsNullOrWhiteSpace(baseURL)
                ? "https://api.openai.com/v1"
                : NormalizeBaseURL(baseURL);
            return NormalizeWireApi(wireApi) == "responses"
                ? $"{endpointBase}/responses"
                : $"{endpointBase}/chat/completions";
        }

        private bool TryReadTomlString(string line, string key, out string value)
        {
            value = "";
            ...
            value = trimmed.Substring(firstQuote + 1, lastQuote - firstQuote - 1);
            return true;
        }

        private string ImportCodexConfig()
        {
            ...
            if (!File.Exists(configPath))
                return $"[Agent] Codex config not found: {configPath}";

            string provider = "";
            string importedModel = "";
            string importedBaseURL = "";
            string importedWireApi = "";
            string importedReasoningEffort = "";
            string importedTextVerbosity = "";
            ...
            foreach (string line in File.ReadAllLines(configPath))
            {
                ...
                {
                    if (TryReadTomlString(trimmed, "model_provider", out string parsedProvider))
                        provider = parsedProvider;
                    else if (TryReadTomlString(trimmed, "model", out string parsedModel))
                        importedModel = parsedModel;
                    else if (TryReadTomlString(trimmed, "model_reasoning_effort", out string parsedReasoningEffort))
                        importedReasoningEffort = parsedReasoningEffort;
                    else if (TryReadTomlString(trimmed, "model_verbosity", out string parsedTextVerbosity))
                        importedTextVerbosity = parsedTextVerbosity;
                    continue;
                }

                if (!string.IsNullOrEmpty(provider) &&
                    string.Equals(currentSection, $"model_providers.{provider}", StringComparison.OrdinalIgnoreCase))
                {
                    if (TryReadTomlString(trimmed, "base_url", out string parsedBaseURL))
                        importedBaseURL = parsedBaseURL;
                    else if (TryReadTomlString(trimmed, "wire_api", out string parsedWireApi))
                        importedWireApi = parsedWireApi;
                }
            }

            if (!string.IsNullOrWhiteSpace(importedModel))
                model = importedModel;
            if (!string.IsNullOrWhiteSpace(importedBaseURL))
                baseURL = NormalizeBaseURL(importedBaseURL);
            if (!string.IsNullOrWhiteSpace(importedWireApi))
                wireApi = NormalizeWireApi(importedWireApi);
            if (!string.IsNullOrWhiteSpace(importedReasoningEffort))
                reasoningEffort = importedReasoningEffort;
            if (!string.IsNullOrWhiteSpace(importedTextVerbosity))
                textVerbosity = importedTextVerbosity;

            SaveSettings();
            return $"[Agent] Imported Codex config. Model: {model}, endpoint: {BuildFinalEndpoint()}, reasoning: {reasoningEffort}, verbosity: {textVerbosity}";
        }
        ...
        private void InitializeScriptManager()
        {
            ...
            scriptManager.Initialize("LLMAgent/editor-assistant", () =>
            {
                ...
                {
                    scriptManager.ConfigureAgent(apiKey, baseURL, model, wireApi, reasoningEffort, textVerbosity, 25);
                }
                Repaint();
            });
        }
        ...
        private void InitStyles()
        {
            ...
            settingsFieldStyle = new GUIStyle(EditorStyles.textField)
            {
                fontSize = 12,
                alignment = TextAnchor.MiddleLeft,
                fixedHeight = SettingsFieldHeight,
        ...
        private void DrawSettingsPanel()
        {
            ...
            apiKey = EditorGUILayout.PasswordField(apiKey, settingsFieldStyle, GUILayout.Height(SettingsFieldHeight));
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(2);
            ...
            baseURL = EditorGUILayout.TextField(baseURL, settingsFieldStyle, GUILayout.Height(SettingsFieldHeight));
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(2);

            EditorGUILayout.BeginHorizontal();
            GUILayout.Label("Wire API", GUILayout.Width(90));
            int selectedWireApi = NormalizeWireApi(wireApi) == "responses" ? 0 : 1;
            selectedWireApi = EditorGUILayout.Popup(
                selectedWireApi,
                new[] { "Responses", "Chat Completions" },
                settingsFieldStyle,
                GUILayout.Height(SettingsFieldHeight));
            wireApi = selectedWireApi == 0 ? "responses" : "chat_completions";
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(2);

            EditorGUILayout.HelpBox(
                string.IsNullOrWhiteSpace(baseURL)
                    ? $"Default endpoint: {BuildFinalEndpoint()}"
                    : $"Final endpoint: {BuildFinalEndpoint()}",
                MessageType.None);
            GUILayout.Space(2);
            ...
            model = EditorGUILayout.TextField(model, settingsFieldStyle, GUILayout.Height(SettingsFieldHeight));
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(2);

            EditorGUILayout.BeginHorizontal();
            GUILayout.Label("Reasoning", GUILayout.Width(90));
            reasoningEffort = EditorGUILayout.TextField(reasoningEffort, settingsFieldStyle, GUILayout.Height(SettingsFieldHeight));
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(2);

            EditorGUILayout.BeginHorizontal();
            GUILayout.Label("Verbosity", GUILayout.Width(90));
            textVerbosity = EditorGUILayout.TextField(textVerbosity, settingsFieldStyle, GUILayout.Height(SettingsFieldHeight));
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(4);
            ...
            if (GUILayout.Button("  Import Codex Config  ", EditorStyles.miniButton))
            {
                string result = ImportCodexConfig();
        ...
        private void ApplySettings()
        {
            if (scriptManager != null && scriptManager.IsInitialized)
            {
                baseURL = NormalizeBaseURL(baseURL);
                wireApi = NormalizeWireApi(wireApi);
                string result = scriptManager.ConfigureAgent(apiKey, baseURL, model, wireApi, reasoningEffort, textVerbosity, 25);
                messages.Add(new ChatMessage
                {
                    Text = result,
                    IsUser = false,
        ...
        {
            string combinedText = !string.IsNullOrWhiteSpace(finalText)
                ? finalText
                : progressFragments.Count > 0
                    ? string.Join("", progressFragments)
                    : "";
